...
@Component({
    selector: 'jhi-domain-assess',
    templateUrl: './domain-assess.component.html'
})
export class DomainAssessComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.domainService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<DomainAssess[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/domain-assess'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/domain-assess', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: DomainAssess) {
        return item.id;
    }
