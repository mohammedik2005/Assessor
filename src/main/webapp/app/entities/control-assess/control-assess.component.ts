...
@Component({
    selector: 'jhi-control-assess',
    templateUrl: './control-assess.component.html'
})
export class ControlAssessComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.controlService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<ControlAssess[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/control-assess'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/control-assess', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ControlAssess) {
        return item.id;
    }
