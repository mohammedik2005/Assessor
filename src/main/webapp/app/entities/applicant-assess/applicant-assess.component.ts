...
@Component({
    selector: 'jhi-applicant-assess',
    templateUrl: './applicant-assess.component.html'
})
export class ApplicantAssessComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.applicantService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<ApplicantAssess[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/applicant-assess'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/applicant-assess', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ApplicantAssess) {
        return item.id;
    }
