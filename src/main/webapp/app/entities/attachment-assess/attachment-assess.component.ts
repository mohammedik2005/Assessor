...
@Component({
    selector: 'jhi-attachment-assess',
    templateUrl: './attachment-assess.component.html'
})
export class AttachmentAssessComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.attachmentService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<AttachmentAssess[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/attachment-assess'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/attachment-assess', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: AttachmentAssess) {
        return item.id;
    }
